...
/**
 * ★ 1호점 5인 개발/아키텍처 감사 위원회 (역할 완전 독립 · 결정론적 규칙 기반)
...
export const DEV_ENGINEERING_AGENTS = [
  ...
  },
  {
    ...
    role: '악성 실행 벡터만 검사: <script> 블록, 인라인 이벤트 핸들러(onerror/onclick 등), javascript: URI 전량 제거. 감점: 발견 유형 1건당 -3점 (제거 후에도 점수는 차감)',
  },
  {
...
/**
 * 1호점 금융/부동산 블로그 5인 개발/아키텍처 감사 수행 (결정론적 규칙 엔진)
 */
export function auditEngineeringAndArchitecture(
  ...
  const balancePairs: Array<[string, string]> = [
    ['div', 'div'], ['p', 'p'], ['table', 'table'], ['ul', 'ul'], ['ol', 'ol'], ['li', 'li'],
  ];
  for (const [tag] of balancePairs) {
  ...
  // =========================================================================
  // 3. [security_xss] 스크립트 / 인라인 핸들러 / javascript: URI 전량 차단
  // =========================================================================
  ...
  feedbacks.push({
    ...
    role: '악성 스크립트 및 인라인 이벤트 핸들러 원천 차단',
    score: clamp(10 - securityIssues.length * 3),
    passed: true,
    issues: securityIssues,
    recommendations: securityIssues.length > 0 ? ['실행 가능 벡터 전량 제거 완료'] : ['XSS 실행 벡터 없음 (클린)'],
  });
